...
class FIRE(nn.Module):
    def __init__(self, num_heads, mlp_width, init_c,
        init_L, eps):
        """
    ...
    def forward(self, rel_distance: torch.Tensor, positions: torch.Tensor):
        """
        ...
        # with log transform
        rel_distance = torch.log1p(
            torch.abs(self.c * rel_distance)
        )
        pos_normalizer = torch.log1p(
            torch.abs(self.c * pos_normalizer)
        ) + self.eps
...
class Attention(nn.Module):
    def __init__(self, args: FIREModelArgs):
        super().__init__()
        ...
        self.fire_bias = FIRE(
            num_heads=args.fire_num_heads,
            mlp_width=args.fire_mlp_width,
            init_c=args.fire_init_c,
            init_L=args.max_seq_len,
            eps=args.fire_eps,
        )
    ...
    def forward(
        ...
        if self.fire is not None:
            scores = scores + self.fire_bias(rel_distance, positions)
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: FIREModelArgs):
        super().__init__()
    ...
    def forward(
        ...
        h = x + self.attention(self.attention_norm(x), mask, rel_distance, positions)
        out = h + self.feed_forward(self.ffn_norm(h))
        return out


class FIRETransformer(nn.Module):
    def __init__(self, params: FIREModelArgs):
        super().__init__()
        self.params = params
    ...
    def forward(self, tokens: torch.Tensor, seq_codes: Optional[torch.Tensor] = None):
        ...
        for layer in self.layers:
            h = layer(h, mask, rel_distance, positions)
        h = self.norm(h)
